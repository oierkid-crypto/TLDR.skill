...
SRC_ROOT = ROOT / "prototypes" / "video_transcript_digest" / "src"

sys.path.insert(0, str(SRC_ROOT))
sys.path.insert(0, str(ROOT))

import video_digest
from video_digest import DigestResult, TranscriptResult, VideoTranscriptDigestError
...
def test_process_video_optimizes_transcript_before_summary() -> None:
    optimized = TranscriptResult(
        source="optimized",
        text="这是优化后的转录稿。",
        raw_text="偷肯 AIDC 没标点",
        title="标题",
    )
    ...
    assert result.summary == "总结输出"
    assert result.reality_check == "核查输出"
...
def test_cli_writes_markdown_file(tmp_path: Path) -> None:
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = f"{SRC_ROOT}:{ROOT}"

    script = (
        "import importlib.util; "
        f"spec=importlib.util.spec_from_file_location('video_digest_cli', r'{SRC_ROOT / 'cli.py'}'); "
        "mod=importlib.util.module_from_spec(spec); spec.loader.exec_module(mod); "
        "from video_digest import DigestResult, TranscriptResult; "
