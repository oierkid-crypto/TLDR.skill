...
from tldr_skill.llm import async_call_llm, extract_content_or_reasoning
from tldr_skill.transcription import _transcribe_local, transcribe_audio
...
def detect_platform(url: str) -> str:
    ...
    if not host:
        raise VideoTranscriptDigestError(f"Cannot detect host from URL: {url}")
    for platform, hosts in SUPPORTED_PLATFORMS.items():
        if host in hosts:
            return platform
    raise VideoTranscriptDigestError("Only Xiaohongshu, Douyin, and YouTube links are supported.")
...
def _import_youtube_transcript_api():
    try:
        ...
        raise VideoTranscriptDigestError("Missing youtube-transcript-api. Install with `pip install -e .[video]`.") from exc


def fetch_youtube_transcript(url: str, languages: list[str] | None = None) -> TranscriptResult:
    video_id = _extract_youtube_video_id(url)
    if not video_id:
        raise VideoTranscriptDigestError(f"Cannot extract YouTube video id from: {url}")
    ...
    try:
        ...
        raise VideoTranscriptDigestError(f"YouTube caption fetch failed: {exc}") from exc

    text = "\n".join(item.get("text", "").strip() for item in transcript_items if item.get("text"))
    if not text.strip():
        raise VideoTranscriptDigestError("YouTube returned an empty transcript.")
...
def _import_yt_dlp():
    try:
        ...
        raise VideoTranscriptDigestError("Missing yt-dlp. Install with `pip install -e .[video]`.") from exc
...
def fetch_youtube_subtitle_transcript(url: str, languages: list[str] | None = None) -> TranscriptResult:
    ...
    with tempfile.TemporaryDirectory(prefix="yt-subs-") as tmp_dir:
        ...
        if not subtitles:
            raise VideoTranscriptDigestError("YouTube auto subtitle download failed.")

        transcript_text = _json3_to_text(str(subtitles[0]))
        if not transcript_text:
            raise VideoTranscriptDigestError("YouTube auto subtitle content is empty.")
...
def _import_requests():
    try:
        ...
        raise VideoTranscriptDigestError("Missing requests dependency.") from exc
...
def _import_playwright():
    try:
        ...
        raise VideoTranscriptDigestError("Missing playwright dependency. Install with `pip install -e .[video]`.") from exc
...
def fetch_douyin_via_browser(url: str, workspace: Path) -> dict[str, Any]:
    ...
    browser_path = os.getenv("TLDR_SKILL_CHROME_PATH") or _default_chrome_executable()
    workspace.mkdir(parents=True, exist_ok=True)
    audio_url: str | None = None
    title: str | None = None

    with sync_playwright() as p:  # pragma: no cover - browser runtime path
        ...
        if not audio_url:
            browser.close()
            raise VideoTranscriptDigestError("Opened the Douyin page but did not capture an audio URL.")
        ...
        if response.status != 200:
            browser.close()
            raise VideoTranscriptDigestError(f"Douyin audio download failed: HTTP {response.status}")

        destination = workspace / "source.m4a"
        destination.write_bytes(response.body())
        browser.close()

    return {"path": str(destination), "title": title, "extractor": "playwright_douyin", "final_url": final_url}
...
def download_media(url: str, workspace: Path) -> dict[str, Any]:
    ...
    if not file_path.exists():
        ...
        if matches:
            file_path = matches[0]
    if not file_path.exists():
        raise VideoTranscriptDigestError("Download finished but no local media file was found.")
...
def transcribe_downloaded_media(media_path: str) -> TranscriptResult:
    ...
    if not transcript:
        raise VideoTranscriptDigestError("Transcript is empty.")
    return TranscriptResult(
...
async def _call_text_model(system_prompt: str, user_prompt: str, *, max_tokens: int = 1400) -> str:
    ...
    if not text:
        raise VideoTranscriptDigestError("LLM returned an empty result.")
    return text
...
async def _optimize_transcript_async(transcript: TranscriptResult) -> TranscriptResult:
    system_prompt = (
        "You are a professional transcript editor. Your job is not to summarize. "
        "Your job is to repair a raw transcript into a readable one while staying faithful to the speaker."
    )
    user_prompt = (
        "Clean the transcript below. Rules:\n"
        "1. Preserve meaning. Do not summarize.\n"
        "2. Fix obvious ASR mistakes, especially mixed Chinese/English terms such as token, AIDC, Agent, webhook.\n"
        "3. Add punctuation and paragraph breaks.\n"
        "4. Keep only the cleaned transcript body in the output.\n\n"
        f"Title: {transcript.title or 'Untitled'}\n"
        f"Raw transcript:\n{transcript.text}"
    )
...
async def summarize_and_reality_check(platform: str, url: str, transcript_text: str) -> tuple[str, str]:
    summary_system = "You write concise, structured Chinese summaries from cleaned transcripts."
    reality_system = "You write concise Chinese Reality Checks: what is credible, what needs caution, and why."
...
def process_video(url: str, *, prefer_youtube_captions: bool = True, languages: list[str] | None = None) -> DigestResult:
    ...
    return DigestResult(url=url, platform=platform, transcript=transcript, summary=summary, reality_check=reality_check)
